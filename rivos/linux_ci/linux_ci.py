...
qemu_cmd = "/rivos/qemu/bin/qemu-system-riscv64 -machine virt -cpu rv64,h=true,len-satp-mode=2,satp-mode[0]=mbare,satp-mode[1]={} -bios fw_dynamic.elf -nographic -m 16G -smp 8 -kernel usr/lib/u-boot/qemu-riscv64_smode/uboot.elf -device virtio-net-device,netdev=net0 -netdev user,hostfwd=tcp::10022-:22,id=net0,tftp=tftp -drive file={},format=raw,if=virtio -virtfs local,path={},mount_tag=host0,security_model=passthrough,id=host0 -device virtio-rng-pci -s"
...
def host_dl_firmware():
    ...
    launch_external_wait("wget --no-verbose --no-check-certificate https://ghiti.fr/nextcloud/index.php/s/bm9mdP2GHEb3jYa/download/fw_dynamic.elf".split(" "))
    print("OK")
