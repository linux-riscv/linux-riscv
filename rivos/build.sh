...
kernel_build() {
    ...
    make LOCALVERSION=$local_version ARCH=riscv CROSS_COMPILE=riscv64-unknown-linux-gnu- O=build_${config} ${config_base}

    if [ ! -z "$config_to_merge" ]; then
        # Avoid a mrproper by stepping into the directory
        cd build_${config}
        ARCH=riscv CROSS_COMPILE=riscv64-unknown-linux-gnu- ../scripts/kconfig/merge_config.sh .config ../$config_to_merge
        cd ..
    fi
