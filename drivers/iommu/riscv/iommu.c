...
static u64 riscv_iommu_ats_inval_payload(unsigned long start,
...
static u64 riscv_iommu_ats_inval_all_payload(bool global_inv)
{
...
static u64 riscv_iommu_ats_prgr_payload(u16 dest_id, u8 resp_code, u16 grp_idx)
{
...
static int riscv_iommu_enable_ir(struct riscv_iommu_endpoint *ep)
{
	...
	/* Enforce in-memory msi redirect info ordering before setting it valid */
	wmb();
...
static void riscv_iommu_disable_ir(struct riscv_iommu_endpoint *ep)
{
	...
	/* Enforce in-memory ordering after setting the entry invalid */
	wmb();
				     ...
				     struct device *dev, ioasid_t pasid)
{
	...
	ep->pc[pasid].fsc = (u64)virt_to_pfn(iommu_domain->mm->pgd) |
			    FIELD_PREP(RISCV_IOMMU_DC_FSC_MODE, domain->mode);

	/* Enforce in-memory process context write ordering before marking it valid */
	wmb();
	ta = RISCV_IOMMU_PC_TA_V | FIELD_PREP(RISCV_IOMMU_PC_TA_PSCID, domain->pscid);
	ta = xchg_relaxed((unsigned long *) &ep->pc[pasid].ta, ta);

	if (ta & RISCV_IOMMU_PC_TA_V) {
		/* Invalidate IOATC only for PC updates. */
		riscv_iommu_iodir_inv_pasid(ep->iommu, ep->devid, pasid);
		riscv_iommu_iofence_sync(ep->iommu);
	}

	dev_dbg(dev, "domain type %d attached w/ PSCID %u PASID %u\n",
...
static void riscv_iommu_remove_dev_pasid(struct device *dev, ioasid_t pasid)
{
	struct riscv_iommu_endpoint *ep = dev_iommu_priv_get(dev);
	struct riscv_iommu_command cmd;
	u64 payload = riscv_iommu_ats_inval_all_payload(false);
	unsigned int pscid;
	unsigned long ta;

	/* invalidate TA.V */
	ta = xchg_relaxed((unsigned long *) &ep->pc[pasid].ta, 0);
	pscid = (unsigned int)FIELD_GET(RISCV_IOMMU_PC_TA_PSCID, ta);

	/* Enforce in-memory process context write ordering before sending IODIR.INV */
	wmb();
	...
	dev_dbg(dev, "domain removed w/ PSCID %u PASID %u\n", pscid, pasid);
}
