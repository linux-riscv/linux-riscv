...
#define RISCV_IOMMU_INVAL_TIMEOUT	(timeout ? timeout : 90000000)
				  ...
				  unsigned int timeout_us)
{
	...
	return riscv_iommu_readl_timeout(queue->iommu, Q_HEAD(queue),
					 head, !(head & ~queue->mask) &&
					 (int)(cons + ((head - last) & queue->mask) - index) > 0,
					 0, timeout_us);
}
