...
static void riscv_iommu_pt_walk_free(pmd_t *ptp, unsigned int shift, bool root)
{
	pmd_t *pte, *pt_base;
	int i;
	...
	for (i = 0; i < PTRS_PER_PMD; i++) {
		...
		if (pmd_present(*pte) && !pmd_leaf(*pte))
			riscv_iommu_pt_walk_free(pte, shift - PT_SHIFT, false);
	}
					...
					gfp_t gfp)
{
	...
	if (root)
		pte = ptp + ((iova >> shift) & (PTRS_PER_PGD - 1));
	else
		pte = (pmd_t *)pfn_to_virt(__page_val_to_pfn(pmd_val(*ptp))) +
		    ((iova >> shift) & (PTRS_PER_PTE - 1));
	...
	return riscv_iommu_pt_walk_alloc(pte, iova, shift - PT_SHIFT, false,
					 pgsize, pd_alloc, gfp);
}
					...
					bool root)
{
	...
	return riscv_iommu_pt_walk_fetch(pte, iova, shift - PT_SHIFT, false);
}
						    ...
						    void *cookie)
{
	...
	cfg->ias = VA_BITS;
	cfg->oas = FIELD_GET(RISCV_IOMMU_CAP_PAS, domain->iommu->cap);
