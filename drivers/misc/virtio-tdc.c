...
static struct ioctl_dma_command {
    uint64_t src;
    uint64_t dst;
    uint64_t size;
...
static void send_dma_request(struct virttdc_info* vi, struct ioctl_dma_command* cmd) {
    ...
    printk("cmd is: %lx\n", cmd);
    printk("size of *cmd is: %d\n", sizeof(*cmd));
    sg_init_one(&sg, cmd, sizeof(*cmd));
    int err = virtqueue_add_outbuf(vi->vq, &sg, 1, cmd, GFP_KERNEL); //TODO: wha'ts the use of data  here 
    if(err < 0){
        printk("virtqueue_add_outbuf failed: %d\n", err);
    }
    int res = virtqueue_kick(vi->vq);
...
static int virttdc_probe(struct virtio_device *vdev) {
    ...
    struct device* chr_dev = device_create(tdc_class, &vdev->dev, MKDEV(MAJOR(dev_num), 0), NULL, VIRTIO_TDC_DEVICE_NAME);
