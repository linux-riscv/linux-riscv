...
struct tdc_hashtable_entry* get_cached_entry(uint64_t va) {
    ...
    hash_for_each_possible(va_map, entry, ht_link, va) {
        if(entry->va==va){
            printk(KERN_INFO "Hash entry found for va %lx -> %lx\n", va, entry->iova);
            return entry;
        }
    }
    return NULL;
...
long tdc_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    switch(cmd) {
        case TDC_DMA_F: {
            ...
            uint32_t status = readl(tdev->regs + TDC_REG_STATUS);
            struct page *src_pages, *dst_pages;
            uint64_t npages = 1; //TODO: change to actual sioze
            down_read(&current->mm->mmap_lock);
            int src_res = get_user_pages((uint64_t) ioctl_dma_cmd.src, npages, 0, &src_pages, NULL);
            
            int dst_res = get_user_pages((uint64_t) ioctl_dma_cmd.dst, npages, FOLL_WRITE, &dst_pages, NULL);
            ...
            for(uint64_t n = 0; n < npages; n++){
                uint64_t src_va = get_iova_cached_or_create(tdev->dev, ioctl_dma_cmd.src + n * PAGE_SIZE, src_pages+n, DMA_TO_DEVICE, 1);        
                uint64_t dst_va = get_iova_cached_or_create(tdev->dev, ioctl_dma_cmd.dst + n * PAGE_SIZE, dst_pages+n, DMA_FROM_DEVICE, 1);
        ...
        case TDC_UNMAP_ENTRY: {
            ...
            for(uint64_t n = 0; n < npages; n++){
                ...
                if(entry==NULL){
                    ...
                    dma_unmap_page(tdev->dev, iova, PAGE_SIZE, DMA_BIDIRECTIONAL); //TODO: better thing than bidirectoinal?
