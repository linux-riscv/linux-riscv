...
static const char *isbdm_qp_state_to_string[ISBDM_QP_STATE_COUNT] = {
	[ISBDM_QP_STATE_IDLE] = "IDLE",
	[ISBDM_QP_STATE_RTR] = "RTR",
	[ISBDM_QP_STATE_RTS] = "RTS",
	[ISBDM_QP_STATE_CLOSING] = "CLOSING",
	[ISBDM_QP_STATE_TERMINATE] = "TERMINATE",
	[ISBDM_QP_STATE_ERROR] = "ERROR"
};
...
// 		   isbdm_qp_state_to_string[qp->attrs.state]);
...
static int isbdm_qp_readq_init(struct isbdm_qp *qp, int irq_size, int orq_size)
{
	if (irq_size) {
		irq_size = roundup_pow_of_two(irq_size);
		qp->irq = vzalloc(irq_size * sizeof(struct isbdm_sqe));
		if (!qp->irq) {
			qp->attrs.irq_size = 0;
			return -ENOMEM;
		}
	...
	if (orq_size) {
		...
		qp->orq = vzalloc(orq_size * sizeof(struct isbdm_sqe));
		if (!qp->orq) {
			qp->attrs.orq_size = 0;
			qp->attrs.irq_size = 0;
			vfree(qp->irq);
 ...
 */
static void isbdm_qp_modify_nonstate(struct isbdm_qp *qp,
				     struct isbdm_qp_attrs *attrs,
				     enum isbdm_qp_attr_mask mask)
{
	if (mask & ISBDM_QP_ATTR_ACCESS_FLAGS) {
		if (attrs->flags & ISBDM_RDMA_BIND_ENABLED)
			qp->attrs.flags |= ISBDM_RDMA_BIND_ENABLED;
		else
			qp->attrs.flags &= ~ISBDM_RDMA_BIND_ENABLED;

		if (attrs->flags & ISBDM_RDMA_WRITE_ENABLED)
			qp->attrs.flags |= ISBDM_RDMA_WRITE_ENABLED;
		else
			qp->attrs.flags &= ~ISBDM_RDMA_WRITE_ENABLED;

		if (attrs->flags & ISBDM_RDMA_READ_ENABLED)
			qp->attrs.flags |= ISBDM_RDMA_READ_ENABLED;
		else
			qp->attrs.flags &= ~ISBDM_RDMA_READ_ENABLED;
	}

	if (mask & ISBDM_QP_ATTR_DEST_QP_NUM)
		qp->attrs.dest_qp_num = attrs->dest_qp_num;

	if (mask & ISBDM_QP_ATTR_AV)
		rdma_replace_ah_attr(&qp->remote_ah_attr, &attrs->ah_attr);
}

/* Attempt a requested state transition when the current state is idle. */
static int isbdm_qp_nextstate_from_idle(struct isbdm_qp *qp,
					struct isbdm_qp_attrs *attrs,
					enum isbdm_qp_attr_mask mask)
{
	int rv = 0;
	...
	switch (attrs->state) {
	case ISBDM_QP_STATE_RTS:
		/* TODO: CRC is not a thing for us. Delete? */
		// if (attrs->flags & ISBDM_MPA_CRC) {
		// 	rv = siw_qp_enable_crc(qp);
		// 	if (rv)
		// 		break;
		// }

		// if (!(mask & ISBDM_QP_ATTR_LLP_HANDLE)) {
		// 	isbdm_dbg_qp(qp, "no socket\n");
		// 	rv = -EINVAL;
		// 	break;
		// }

		// if (!(mask & ISBDM_QP_ATTR_MPA)) {
		// 	isbdm_dbg_qp(qp, "no MPA\n");
		// 	rv = -EINVAL;
		// 	break;
		// }

		/* TODO: Do these need to get replaced with anything? */
		// /*
		 ...
		 * limits.
		 */
		rv = isbdm_qp_readq_init(qp, attrs->irq_size,
					 attrs->orq_size);
		if (rv)
			break;
...
/*
 * Attempt a requested state transition when the current state is Ready To Send.
 */
static int isbdm_qp_nextstate_from_rts(struct isbdm_qp *qp,
				       struct isbdm_qp_attrs *attrs)
{
	int drop_conn = 0;
	...
	case ISBDM_QP_STATE_TERMINATE:
		qp->attrs.state = ISBDM_QP_STATE_TERMINATE;
		/* TODO: Is this or a replacement for it needed? */
		// isbdm_init_terminate(qp, TERM_ERROR_LAYER_RDMAP,
...
/* Attempt a requested state transition when the current state is terminate. */
static void isbdm_qp_nextstate_from_term(struct isbdm_qp *qp,
					 struct isbdm_qp_attrs *attrs)
{
	switch (attrs->state) {
	case ISBDM_QP_STATE_ERROR:
...
/* Attempt a requested state transition when the current state is close. */
static int isbdm_qp_nextstate_from_close(struct isbdm_qp *qp,
					 struct isbdm_qp_attrs *attrs)
{
	int rv = 0;
	...
	default:
		isbdm_dbg_qp(qp, "state transition undefined: %s => %s\n",
			   isbdm_qp_state_to_string[qp->attrs.state],
			   isbdm_qp_state_to_string[attrs->state]);
 ...
 */
int isbdm_qp_modify(struct isbdm_qp *qp, struct isbdm_qp_attrs *attrs,
		    enum isbdm_qp_attr_mask mask)
{
	...
	isbdm_dbg_qp(qp, "state: %s => %s\n",
		     isbdm_qp_state_to_string[qp->attrs.state],
		     isbdm_qp_state_to_string[attrs->state]);

	if (mask != ISBDM_QP_ATTR_STATE)
		isbdm_qp_modify_nonstate(qp, attrs, mask);
	...
	switch (qp->attrs.state) {
	case ISBDM_QP_STATE_IDLE:
	case ISBDM_QP_STATE_RTR:
		rv = isbdm_qp_nextstate_from_idle(qp, attrs, mask);
		break;

	case ISBDM_QP_STATE_RTS:
		drop_conn = isbdm_qp_nextstate_from_rts(qp, attrs);
		break;

	case ISBDM_QP_STATE_TERMINATE:
		isbdm_qp_nextstate_from_term(qp, attrs);
		break;

	case ISBDM_QP_STATE_CLOSING:
		isbdm_qp_nextstate_from_close(qp, attrs);
		break;
	...
	if (drop_conn) {
		/* TODO: Is this or some replacement for it needed? */
		// siw_qp_cm_drop(qp, 0);
	}
 ...
 */
static bool isbdm_cq_notify_now(struct isbdm_cq *cq, u32 flags)
{
	u32 cq_notify;
	...
	if ((cq_notify & ISBDM_NOTIFY_NEXT_COMPLETION) ||
	    ((cq_notify & ISBDM_NOTIFY_SOLICITED) &&
	     (flags & ISBDM_WQE_SOLICITED))) {

		/*
		 * CQ notification is one-shot: Since the
		 * current CQE causes user notification,
		 * the CQ gets disarmed and must be rearmed
		 * by the user for a new notification.
		 */
		WRITE_ONCE(cq->notify->flags, ISBDM_NOTIFY_NOT);
		return true;
	}
		       ...
		       enum isbdm_wc_status status)
{
	...
	if (!READ_ONCE(cqe->flags)) {
		...
		WRITE_ONCE(cqe->flags, ISBDM_WQE_VALID);

		/* recycle SQE */
		smp_store_mb(sqe->flags, 0);
		cq->cq_put++;
		notify = isbdm_cq_notify_now(cq, sqe_flags);
		spin_unlock_irqrestore(&cq->lock, flags);
		if (notify) {
			isbdm_dbg_cq(cq,
				"Call completion handler, qp=%x id=%llx\n",
				qp_id(qp), sqe->id);
		...
		spin_unlock_irqrestore(&cq->lock, flags);
		rv = -ENOMEM;
		isbdm_cq_event(cq, IB_EVENT_CQ_ERR);
	}
		       ...
		       enum isbdm_wc_status status)
{
	...
	if (!READ_ONCE(cqe->flags)) {
		bool notify;
		u8 cqe_flags = ISBDM_WQE_VALID;

		cqe->id = rqe->id;
		cqe->opcode = ISBDM_OP_RECEIVE;
		cqe->status = status;
		cqe->imm_data = 0;
		cqe->bytes = bytes;
		if (rdma_is_kernel_res(&cq->base_cq.res)) {
		...
		smp_store_mb(rqe->flags, 0);
		cq->cq_put++;
		notify = isbdm_cq_notify_now(cq, ISBDM_WQE_SIGNALLED);
		spin_unlock_irqrestore(&cq->lock, flags);
		if (notify) {
			isbdm_dbg_cq(cq, "Call completion handler\n");
			cq->base_cq.comp_handler(&cq->base_cq,
					cq->base_cq.cq_context);
		}
	} else {
...
void isbdm_sq_flush(struct isbdm_qp *qp)
{
	...
	while (qp->attrs.orq_size) {
		...
		if (isbdm_sqe_complete(qp, sqe, 0, ISBDM_WC_WR_FLUSH_ERR) != 0)
			break;
	...
	if (async_event)
		isbdm_qp_event(qp, IB_EVENT_SQ_DRAINED);
}
...
{
	struct isbdm_qp *found, *qp = container_of(ref, struct isbdm_qp, ref);
	struct isbdm_device *sdev = qp->sdev;
	unsigned long flags;
